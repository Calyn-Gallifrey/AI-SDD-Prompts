...
1. 不得只依赖 `git diff`，因为新增未跟踪文件不会出现在普通 diff 中。
2. 必须用 `git status --short`、`rg --files` 或等价方式识别本次任务范围内的新增文件。
3. 未跟踪但位于 `tasks.md` 允许修改范围内的源码、测试、配置和 SDD 产物必须纳入评审。
4. 未跟踪且不在允许范围内的文件必须标记为范围偏差或要求说明。

如果无法确定本次实现范围，必须停止并要求补充变更文件清单、Git Diff 范围或未跟踪文件说明。
