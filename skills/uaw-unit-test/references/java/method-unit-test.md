```
**任务**
针对[用户选中代码]按照以下步骤生成单元测试用例。

**示例边界**
本文中的代码片段、类名、字段名、测试数据和业务文本仅用于说明方法级单元测试结构和写法，不是当前任务的默认测试目标。生成测试时，必须基于用户选中代码、当前代码和已确认测试目标替换所有示例业务信息。
...
4. 生成测试用例：
  ...
  - 生成单元测试用例时必须严格遵守[基本要求]与[团队自定义要求](如果有)
...
8. 聚焦结果：只返回生成的单测用例，以 markdown 格式展示
