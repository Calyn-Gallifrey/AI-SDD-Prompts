# 目标
本规则用于根据用户描述编写 DDL 语句，并写入指定目录和文件。

## 示例边界

本文件中的目录、文件名、表名、字段名、日期、环境和脚本示例仅用于说明 DDL 与部署说明的结构，不是当前任务的默认值。生成脚本时，必须基于当前 Brief Design、数据库设计和用户确认信息替换所有示例内容。
...
- **表设计规范**：
  ...
  - 表名必须以 `iic_crm_{业务实体}` 格式命名，如 `iic_crm_transaction_change_phones_detail`
...
* 修改部署说明文件，在部署说明文件 json 的 mysql 列表末尾追加包含路径地址的脚本名称字符串；部署说明文件只能追加修改，不得覆盖。
```
部署说明文件示例
{
  "sqls": {
...
## 执行安全规则
1. 如果用户输入缺少必要信息（需求ID、应用工程、环境、迭代编号、表名、字段信息、提交人等），必须停止执行并列出缺失信息，不得猜测。
2. 如果deploy_desc JSON文件不存在或无法解析，必须停止执行并提示错误，不得创建新的deploy_desc文件。
3. 文件操作仅允许在db目录及其子目录下执行，禁止修改其他工程目录。
4. 只允许根据用户明确提供的信息生成DDL或ALTER语句，不得假设不存在的字段、表名或业务含义。
